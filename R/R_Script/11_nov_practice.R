...
m3[, 1:2] ## [ row : column ] only first and second column but all rows

m3[3:4, ] ## [ row : column] only 3rd and 4th row but all columns
...
HairEyeColor["Red", , , drop=FALSE] ## preserve the original dimension
HairEyeColor["Red", , ] ### original dimension is violated

mode(HairEyeColor)

HairEyeColor[1, 1, 1] ## haie eye sex
HairEyeColor[1, 1, 1] <- "50"
HairEyeColor
mode(HairEyeColor)

# direct
(an_array <- array(1:20, dim = c(2,5,2))) # 2 row 5 column 2 catagory
...
lapply(grndparent_list$first$another_list, sum) ### keeps the original structure

sapply(grndparent_list$first$another_list, sum) ### dropping of dimension
...
## combining information from elements of seperate lists
...
str(df1)
df1[3, "Age"] # age value of 3rd index

mean(df1$Income)

subset(df1, Imputed == TRUE) ## subset of matrix which have value true in imputed column

?subset.default
##################################################################


## numeric vector to factor vector
##############################################################
df1$age_groupe <- cut(
                df1$Age, 
                breaks = c(0,25, 50, 100),
                labels = c("Young", "Middle", "Old"),
                right = TRUE) #### new column in df1
...
proper_list <- split(ToothGrowth, ToothGrowth$supp) ### need only one element
str(proper_list)
